...
public class VoteExecutiveProcessor {

  public void validate(VoteExecutiveParam param, Repository repo) throws ContractValidateException {
    if (repo == null) {
  ...
  public void execute(VoteExecutiveParam param, Repository repo) throws ContractExeException {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        byte[] executiveAddress = vote.getVoteAddress().toByteArray();
        /*
          Already covered while doing maintenance in MaintenanceManager.java, for svm performance,
          we remove the account check
         */
//        if (repo.getAccount(executiveAddress) == null) {
//          throw new ContractValidateException(
//              ACCOUNT_EXCEPTION_STR + StringUtil.encode58Check(executiveAddress) + NOT_EXIST_STR);
//        }
        if (repo.getExecutive(executiveAddress) == null) {
          throw new ContractExeException(
              EXECUTIVE_EXCEPTION_STR + StringUtil.encode58Check(executiveAddress) + NOT_EXIST_STR);
        }
