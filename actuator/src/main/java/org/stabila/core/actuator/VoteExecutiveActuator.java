...
public class VoteExecutiveActuator extends AbstractActuator {


  public VoteExecutiveActuator() {
    super(ContractType.VoteExecutiveContract, VoteExecutiveContract.class);
  }
  ...
  public boolean execute(Object object) throws ContractExeException {
    ...
    long fee = calcFee();
    try {
      VoteExecutiveContract voteContract = any.unpack(VoteExecutiveContract.class);
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (!this.any.is(VoteExecutiveContract.class)) {
      throw new ContractValidateException(
          "contract type error, expected type [VoteExecutiveContract], real type[" + any
              .getClass() + "]");
    }
    final VoteExecutiveContract contract;
    try {
      contract = this.any.unpack(VoteExecutiveContract.class);
    } catch (InvalidProtocolBufferException e) {
      logger.debug(e.getMessage(), e);
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        byte[] executiveCandidate = vote.getVoteAddress().toByteArray();
        if (!DecodeUtil.addressValid(executiveCandidate)) {
        ...
        String readableExecutiveAddress = StringUtil.createReadableString(vote.getVoteAddress());
        if (!accountStore.has(executiveCandidate)) {
          throw new ContractValidateException(
              ACCOUNT_EXCEPTION_STR + readableExecutiveAddress + NOT_EXIST_STR);
        }
        if (!executiveStore.has(executiveCandidate)) {
          throw new ContractValidateException(
              EXECUTIVE_EXCEPTION_STR + readableExecutiveAddress + NOT_EXIST_STR);
        }
  ...
  private void countVoteAccount(VoteExecutiveContract voteContract) {
    AccountStore accountStore = chainBaseManager.getAccountStore();
  ...
  public ByteString getOwnerAddress() throws InvalidProtocolBufferException {
    return any.unpack(VoteExecutiveContract.class).getOwnerAddress();
  }
