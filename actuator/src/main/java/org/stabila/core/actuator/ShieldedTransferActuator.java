...
import static org.stabila.core.capsule.TransactionCapsule.getShieldTransactionHashIgnoreTypeException;
...
import org.stabila.core.exception.ZkProofValidateException;
...
public class ShieldedTransferActuator extends AbstractActuator {
  ...
  public boolean execute(Object result)
    ...
    try {
      ...
      Commons.adjustAssetBalanceV2(accountStore.getUnit(),
  ...
  private void checkProof(List<SpendDescription> spendDescriptions,
    ...
    if (CollectionUtils.isNotEmpty(spendDescriptions)
      ...
      try {
        ...
        for (ReceiveDescription receiveDescription : receiveDescriptions) {
          if (receiveDescription.getCEnc().size() != ZenChainParams.ZC_ENCCIPHERTEXT_SIZE
              || receiveDescription.getCOut().size() != ZenChainParams.ZC_OUTCIPHERTEXT_SIZE) {
