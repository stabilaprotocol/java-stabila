...
@Component
@Slf4j(topic = "API")
public class UnCdAssetServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      UncdAssetContract.Builder build = UncdAssetContract.newBuilder();
      JsonFormat.merge(params.getParams(), build, params.isVisible());
      Transaction tx = wallet
          .createTransactionCapsule(build.build(), ContractType.UncdAssetContract)
          .getInstance();
