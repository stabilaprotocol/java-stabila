...
function getTokenBalanceConstant(address addr, srcToken tokenId) constant returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function getTokenBalance(address addr, srcToken tokenId) returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function transferTokenInAssembly(address addr, srcToken tokenId, uint256 tokenValue) payable {
...
function srcTokenInMap(srcToken tokenId, uint256 tokenValue) returns(uint256 r) {
tokenCnt[tokenId] += tokenValue;
r = tokenCnt[tokenId];
}

function cntTokenTokenInMap(srcToken tokenId1, srcToken tokenId2, uint256 tokenValue) returns(srcToken r) {
