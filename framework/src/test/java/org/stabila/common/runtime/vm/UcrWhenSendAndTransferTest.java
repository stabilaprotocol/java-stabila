...
@Slf4j
public class UcrWhenSendAndTransferTest {

  private Manager dbManager;
  private StabilaApplicationContext context;
  private DepositImpl deposit;
  private String dbPath = "output_UcrWhenSendAndTransferTest";
  ...
  public void callValueTest()
    ...
    Assert.assertEquals(dbManager.getAccountStore().get(address).getBalance(),
        totalBalance - value - expectUcrUsageTotal * 100);

    /* =================================== CALL simpleCall() =================================== */
    byte[] triggerData = SvmTestUtils.parseAbi("simpleCall()", null);
    result = SvmTestUtils
        .triggerContractAndReturnSvmTestResult(Hex.decode(OWNER_ADDRESS), contractAddress,
            triggerData, 0, feeLimit, dbManager, null);

    long expectUcrUsageTotal2 = 7370;
    Assert.assertEquals(result.getReceipt().getUcrUsageTotal(), expectUcrUsageTotal2);
    Assert.assertEquals(dbManager.getAccountStore().get(address).getBalance(),
        totalBalance - value - (expectUcrUsageTotal + expectUcrUsageTotal2) * 100);

    /* =================================== CALL complexCall() =================================== */
    triggerData = SvmTestUtils.parseAbi("complexCall()", null);
    result = SvmTestUtils
        .triggerContractAndReturnSvmTestResult(Hex.decode(OWNER_ADDRESS), contractAddress,
            triggerData, 0, feeLimit, dbManager, null);

    long expectUcrUsageTotal3 = 9459;
  ...
  public void sendTest()
    ...
    /* =================================== CALL doSend() =================================== */
    byte[] triggerData = SvmTestUtils.parseAbi("doSend()", null);
    result = SvmTestUtils
        .triggerContractAndReturnSvmTestResult(Hex.decode(OWNER_ADDRESS), contractAddress,
            triggerData, 0, feeLimit, dbManager, null);

    long expectUcrUsageTotal2 = 7025;
    Assert.assertEquals(result.getReceipt().getUcrUsageTotal(), expectUcrUsageTotal2);
  ...
  public void transferTest()
    ...
    /* =================================== CALL doSend() =================================== */
    byte[] triggerData = SvmTestUtils.parseAbi("doTransfer()", null);
    result = SvmTestUtils
        .triggerContractAndReturnSvmTestResult(Hex.decode(OWNER_ADDRESS), contractAddress,
            triggerData, 0, feeLimit, dbManager, null);

    long expectUcrUsageTotal2 = 7030;
    Assert.assertEquals(result.getReceipt().getUcrUsageTotal(), expectUcrUsageTotal2);
    Assert.assertEquals(result.getRuntime().getResult().getException(), null);
    Assert.assertEquals(result.getRuntime().getResult().isRevert(), true);
    Assert.assertEquals(deposit.getAccount(contractAddress).getBalance(), value);
  ...
  public SVMTestResult deployCallValueTestContract(long value, long feeLimit,
  ...
  public SVMTestResult deploySendAndTransferTestContract(long value, long feeLimit,
      long consumeUserResourcePercent)
      throws ContractExeException, ReceiptCheckErrException, ContractValidateException,
      VMIllegalException {
