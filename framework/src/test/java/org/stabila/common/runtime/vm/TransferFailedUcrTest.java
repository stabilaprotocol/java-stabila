...
public class TransferFailedUcrTest extends VMTestBase {
  /*
  ...
  */
  ...
  TestCase[] testCasesAfterAllowSvmConstantinop = {
  ...
  TestCase[] testCasesBeforeAllowSvmConstantinop = {
  ...
  @Test
  public void testTransferFailedAfterAllowSvmConstantinopl()
    ...
    for (TestCase testCase : testCasesAfterAllowSvmConstantinop) {
  ...
  @Test
  public void testTransferFailedBeforeAllowSvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowSvmTransferTrc10(1);
    ...
    for (TestCase testCase : testCasesBeforeAllowSvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }
  ...
  private void checkResult(TestCase testCase, byte[] factoryAddress)
    ...
    Assert.assertEquals(receiptCapsule.getResult(), testCase.getReceiptResult(),
        testCase.getMethod());
    if (testCase.allUcr) {
      Assert.assertEquals(programResult.getUcrUsed(), 1000000, testCase.getMethod());
    } else {
      Assert.assertTrue(programResult.getUcrUsed() < allUcr, testCase.getMethod());
    }
  }
