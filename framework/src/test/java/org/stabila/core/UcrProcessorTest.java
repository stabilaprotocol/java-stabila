...
@Slf4j
public class UcrProcessorTest {

  private static final String dbPath = "UcrProcessorTest";
  ...
  @Test
  public void testUseContractCreatorUcr() throws Exception {
    ...
    UcrProcessor processor = new UcrProcessor(dbManager.getDynamicPropertiesStore(),
        dbManager.getAccountStore());
    long ucr = 10000;
    long now = 1526647838000L;

    boolean result = processor.useUcr(ownerCapsule, ucr, now);
    Assert.assertEquals(false, result);

    ownerCapsule.setCdedForUcr(10_000_000L, 0L);
    result = processor.useUcr(ownerCapsule, ucr, now);
    ...
    Assert.assertEquals(1526647838000L,
        ownerCapsuleNew.getAccountResource().getLatestConsumeTimeForUcr());
    Assert.assertEquals(10000L, ownerCapsuleNew.getAccountResource().getUcrUsage());
  ...
  public void updateAdaptiveTotalUcrLimit() {
    ...
    // open
    dbManager.getDynamicPropertiesStore().saveAllowAdaptiveUcr(1);

    // Test resource usage auto reply
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(1526647838000L);
    long now = chainBaseManager.getHeadSlot();
    dbManager.getDynamicPropertiesStore().saveTotalUcrAverageTime(now);
    dbManager.getDynamicPropertiesStore().saveTotalUcrAverageUsage(4000L);
    ...
    processor.updateTotalUcrAverageUsage();
    Assert.assertEquals(2000L,
        dbManager.getDynamicPropertiesStore().getTotalUcrAverageUsage());

    // test saveTotalUcrLimit
    long ratio = ChainConstant.WINDOW_SIZE_MS / AdaptiveResourceLimitConstants.PERIODS_MS;
    dbManager.getDynamicPropertiesStore().saveTotalUcrLimit(10000L * ratio);
    Assert.assertEquals(1000L,
        dbManager.getDynamicPropertiesStore().getTotalUcrTargetLimit());

    //Test exceeds resource limit
    dbManager.getDynamicPropertiesStore().saveTotalUcrCurrentLimit(10000L * ratio);
    dbManager.getDynamicPropertiesStore().saveTotalUcrAverageUsage(3000L);
    processor.updateAdaptiveTotalUcrLimit();
    Assert.assertEquals(10000L * ratio,
        dbManager.getDynamicPropertiesStore().getTotalUcrCurrentLimit());

    //Test exceeds resource limit 2
    dbManager.getDynamicPropertiesStore().saveTotalUcrCurrentLimit(20000L * ratio);
    dbManager.getDynamicPropertiesStore().saveTotalUcrAverageUsage(3000L);
    processor.updateAdaptiveTotalUcrLimit();
    ...
    dbManager.getDynamicPropertiesStore().saveTotalUcrCurrentLimit(20000L * ratio);
    dbManager.getDynamicPropertiesStore().saveTotalUcrAverageUsage(500L);
    processor.updateAdaptiveTotalUcrLimit();
