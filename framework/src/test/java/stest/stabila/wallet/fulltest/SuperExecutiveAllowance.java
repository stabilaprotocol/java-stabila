...
@Slf4j
public class SuperExecutiveAllowance {
  ...
  private static final Long costForCreateExecutive = 10009000000L;
  //testng001、testng002、testng003、testng004
  ...
  public void testInvaildToApplyBecomeExecutive() {
    Assert.assertFalse(createExecutive(INVAILD_ADDRESS, createUrl, testKey002));
  }

  //@Test(enabled = true,threadPoolSize = 10, invocationCount = 10)
  @Test(enabled = false)
  public void testCreate130Executive() {
    ...
    while (executiveList.getExecutivesCount() < 130) {
      ECKey ecKey = new ECKey(Utils.getRandom());
      byte[] lowBalAddress = ecKey.getAddress();
      String lowBalTest = ByteArray.toHexString(ecKey.getPrivKeyBytes());
      logger.info(lowBalTest);
      ...
      Assert.assertTrue(createExecutive(lowBalAddress, createUrl, lowBalTest));
      String voteStr = Base58.encode58Check(PublicMethed.getFinalAddress(lowBalTest));
      HashMap<String, String> smallVoteMap = new HashMap<String, String>();
      smallVoteMap.put(voteStr, "1");
      Assert.assertTrue(voteExecutive(smallVoteMap, lowBalAddress, lowBalTest));
      executivelist = blockingStubFull
          .listExecutives(GrpcAPI.EmptyMessage.newBuilder().build());
      result = Optional.ofNullable(executivelist);
      executiveList = result.get();
  ...
  public void testQueryAllowance() {
    ...
    for (Integer i = 0; i < executiveList.getExecutivesCount(); i++) {
      /*      executiveList.getExecutives(i).getAddress();
      executiveList.getExecutives(i).getAddress();
      executiveList.getExecutives(i).getAddress();
      executiveList.getExecutives(i).getAddress();*/
      ByteString addressBs = executiveList.getExecutives(i).getAddress();
