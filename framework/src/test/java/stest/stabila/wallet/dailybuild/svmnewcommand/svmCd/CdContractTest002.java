...
@Slf4j
public class CdContractTest002 {
  ...
  @Test(enabled = true, description = "contract cd over balance")
  void CdContract001() {
    ...
    Long cdCount = contractAccount_before.getBalance() + 1;
    ...
    Assert.assertEquals(account002_before.getUcrLimit(), account002_after.getUcrLimit());
  ...
  @Test(enabled = true, description = "contract cd amount < 1 STB")
  void CdContract002() {
    ...
    Assert.assertEquals(
        account002_before.getAccountResource().getAcquiredDelegatedCdedBalanceForUcr(),
        account002_after.getAccountResource().getAcquiredDelegatedCdedBalanceForUcr());
    Assert.assertEquals(contractAccount_before.getAccountResource()
            .getDelegatedCdedBalanceForUcr(),
        contractAccount_after.getAccountResource().getDelegatedCdedBalanceForUcr());
    Assert.assertEquals(contractAccount_before.getBalance(),
  ...
  @Test(enabled = true, description = "contract transfer all balance, then cd")
  void CdContract003() {
    ...
    // cd(address payable receiver, uint amount, uint res)
    Long cdCount = contractAccount_before.getBalance();
    String methedStr = "cdAndSend(address,uint256,uint256)";
    String argsStr = "\"" + Base58.encode58Check(testAddress002) + "\"," + cdCount + "," + "1";
    String txid = PublicMethed.triggerContract(contractAddress,methedStr,argsStr,
        false,0,maxFeeLimit,testAddress001,testKey001,blockingStubFull);
  ...
  @Test(enabled = true, description = "contract cd to ger Net")
  void CdContract004() {
    ...
    // cd(address payable receiver, uint amount, uint res)
    Long cdCount = 1_000000L;
    String methedStr = "cd(address,uint256,uint256)";
    String argsStr = "\"" + Base58.encode58Check(testAddress002) + "\"," + cdCount + "," + "0";
    String txid = PublicMethed.triggerContract(contractAddress,methedStr,argsStr,
        false,0,maxFeeLimit,testAddress001,testKey001,blockingStubFull);
    ...
    Assert.assertEquals(contractAccount_before
            .getDelegatedCdedBalanceForBandwidth() + cdCount,
        contractAccount_after.getDelegatedCdedBalanceForBandwidth());
