...
@Slf4j
public class CreateaAndUpdateExecutive2Test {
  ...
  @Test
  public void testInvaildToApplyBecomeExecutive2() {
    GrpcAPI.Return ret1 = createExecutive2(INVAILD_ADDRESS, createUrl, testKey002);
  ...
  @Test(enabled = true)
  public void testCreateExecutive2() {
    //If you are already is executive, apply failed
    createExecutive(fromAddress, createUrl, testKey002);
    GrpcAPI.Return ret1 = createExecutive2(fromAddress, createUrl, testKey002);
    Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(ret1.getMessage().toStringUtf8(),
        "contract validate error : Executive[415624c12e308b03a1a6b21d9b86e3942fac1ab92b] "
            + "has existed");
    //balance is not enouhg,try to create executive.
    ...
    if (result.get().getExecutivesCount() < 6) {
      Assert.assertTrue(sendcoin(lowBalAddress, costForCreateExecutive, fromAddress, testKey002));
      ret1 = createExecutive2(lowBalAddress, createUrl, lowBalTest);
  ...
  @Test(enabled = true)
  public void testUpdateExecutive2() {
    ExecutiveList executivelist = blockingStubFull
        .listExecutives(GrpcAPI.EmptyMessage.newBuilder().build());
    Optional<ExecutiveList> result = Optional.ofNullable(executivelist);
    ExecutiveList executiveList = result.get();
    if (result.get().getExecutivesCount() < 6) {
      ...
      GrpcAPI.Return ret1 = updateExecutive2(lowBalAddress, wrongUrl, lowBalTest);
      Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
      Assert.assertEquals(ret1.getMessage().toStringUtf8(),
          "contract validate error : Invalid url");
      //Content space and special char, update success
      ret1 = updateExecutive2(lowBalAddress, updateSpaceUrl, lowBalTest);
      Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.SUCCESS);
      Assert.assertEquals(ret1.getMessage().toStringUtf8(), "");
      //update success
      ret1 = updateExecutive2(lowBalAddress, updateUrl, lowBalTest);
      Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.SUCCESS);
      Assert.assertEquals(ret1.getMessage().toStringUtf8(), "");
    } else {
  ...
  public Boolean updateExecutive(byte[] owner, byte[] url, String priKey) {
    ...
    ExecutiveContract.ExecutiveUpdateContract.Builder builder = ExecutiveContract.ExecutiveUpdateContract
        .newBuilder();
    builder.setOwnerAddress(ByteString.copyFrom(owner));
    builder.setUpdateUrl(ByteString.copyFrom(url));
    ExecutiveContract.ExecutiveUpdateContract contract = builder.build();
  ...
  public GrpcAPI.Return updateExecutive2(byte[] owner, byte[] url, String priKey) {
    ECKey temKey = null;
    try {
      BigInteger priK = new BigInteger(priKey, 16);
      temKey = ECKey.fromPrivate(priK);
