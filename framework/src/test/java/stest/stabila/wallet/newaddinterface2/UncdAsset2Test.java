...
public class UncdAsset2Test {
  ...
  public void testGetAllAssetIssue2() {
    ...
    Assert.assertEquals(ret1.getMessage().toStringUtf8(),
        "contract validate error : Cded supply cannot exceed total supply");
    ...
    Assert.assertEquals(ret1.getMessage().toStringUtf8(), "contract validate error : "
        + "cdedDuration must be less than 3652 days and more than 1 days");
    ...
    Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(ret1.getMessage().toStringUtf8(),
        "contract validate error : Cded supply must be greater than 0!");
    ...
    Assert.assertEquals(ret1.getCode(), GrpcAPI.Return.response_code.CONTRACT_VALIDATE_ERROR);
    Assert.assertEquals(ret1.getMessage().toStringUtf8(),
        "contract validate error : StbNum must greater than 0!");
    //StbNum is -1.
    ret1 = PublicMethed.createAssetIssue2(lowBalAddress, name, totalSupply, -1, 10,
        start, end, 2, description, url, 10000L, 10000L,
        1L, 3652L, lowBalTest, blockingStubFull);
    ...
    //CdBalance
    Assert.assertTrue(PublicMethed.cdBalance(lowBalAddress, 10000000L, 3, lowBalTest,
        blockingStubFull));
    //Create success
    ...
    logger.info(Integer.toString(assetIssueList.getAssetIssue(0).getCdedSupplyCount()));
    Assert.assertTrue(assetIssueList.getAssetIssue(0).getCdedSupplyCount() == 1);
    //Assert.assertTrue(assetIssueList.getAssetIssue(j).getCdedSupplyCount() > 0);
    Assert.assertTrue(assetIssueList.getAssetIssue(0).getCdedSupply(0).getCdedAmount() > 0);
    Assert.assertTrue(assetIssueList.getAssetIssue(0).getCdedSupply(0).getCdedDays() > 0);
  ...
  public boolean unCdAsset2(byte[] addRess, String priKey) {
    byte[] address = addRess;
    ...
    GrpcAPI.TransactionExtention transactionExtention = blockingStubFull.uncdAsset2(contract);
