...
# Function: to start, stop and restart java-stabila.
...
JAR_FILE_PATH="./build/libs/java-stabila.jar"
PID_FILE_PATH="java-stabila.pid"
LOG_FILE_PATH="java-stabila.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-stabila."
        else
            echo "Failed to start java-stabila."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-stabila."
        else
            echo "Failed to stop java-stabila."
        fi
    ;;
    restart)
