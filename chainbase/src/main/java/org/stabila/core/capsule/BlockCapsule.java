...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public BlockCapsule(long number, Sha256Hash hash, long when, ByteString executiveAddress) {
    // block header raw
    ...
    BlockHeader.raw blockHeaderRaw = blockHeaderRawBuild
        ...
        .setExecutiveAddress(executiveAddress)
        .build();
  ...
  public BlockCapsule(long timestamp, ByteString parentHash, long number,
    ...
    // block
    Block.Builder blockBuild = Block.newBuilder();
    transactionList.forEach(stb -> blockBuild.addTransactions(stb));
    this.block = blockBuild.setBlockHeader(blockHeader).build();
    initTxs();
  }
  ...
  public void addTransaction(TransactionCapsule pendingStb) {
    this.block = this.block.toBuilder().addTransactions(pendingStb.getInstance()).build();
    getTransactions().add(pendingStb);
  }
  ...
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(stb -> new TransactionCapsule(stb))
  ...
  public void sign(byte[] privateKey) {
    ...
    ByteString sig = ByteString.copyFrom(ecKeyEngine.Base64toBytes(ecKeyEngine.signHash(getRawHash()
        .getBytes())));
    BlockHeader blockHeader = this.block.getBlockHeader().toBuilder().setExecutiveSignature(sig)
        .build();
    ...
    try {
      byte[] sigAddress = SignUtils.signatureToAddress(getRawHash().getBytes(),
          TransactionCapsule.getBase64FromByteString(
              block.getBlockHeader().getExecutiveSignature()),
          CommonParameter.getInstance().isECKeyCryptoEngine());
      byte[] executiveAccountAddress = block.getBlockHeader().getRawData().getExecutiveAddress()
          .toByteArray();

      if (dynamicPropertiesStore.getAllowMultiSign() != 1) {
        return Arrays.equals(sigAddress, executiveAccountAddress);
      } else {
        byte[] executivePermissionAddress = accountStore.get(executiveAccountAddress)
  ...
  public void setExecutive(String executive) {
    BlockHeader.raw blockHeaderRaw =
        this.block.getBlockHeader().getRawData().toBuilder().setExecutiveAddress(
            ByteString.copyFrom(executive.getBytes())).build();
  ...
  public ByteString getExecutiveAddress() {
    return this.block.getBlockHeader().getRawData().getExecutiveAddress();
  }
  ...
  public boolean hasExecutiveSignature() {
    return !getInstance().getBlockHeader().getExecutiveSignature().isEmpty();
  }
  ...
  public String toString() {
    ...
    toStringBuff.append("executive address=")
        .append(ByteArray.toHexString(getExecutiveAddress().toByteArray())).append("\n");
